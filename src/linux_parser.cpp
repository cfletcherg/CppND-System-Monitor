...
long LinuxParser::IdleJiffies()
{
  ...
  auto values = CpuUtilization();
  idle_jiffies +=
      std::stol(values[CPUStates::kIdle_]) + std::stol(values[CPUStates::kIOwait_]);
  return idle_jiffies;
}
...
{
  vector<string> cpu;
  string         line_name, user, nice, system, idle, iowait, irq, softirq, steal, guest,
      guest_nice;
  string        line{};
  ...
  if (stream.is_open())
  {
    std::getline(stream, line);
    std::istringstream linestream(line);
    linestream >> line_name >> user >> nice >> system >> idle >> iowait >> irq >>
...
string LinuxParser::Ram(int pid)
{
  ...
  if (stream.is_open())
  {
    while (std::getline(stream, line))
    {
      ...
      if (line_name == "VmSize:")
      {
        long ram_val = std::stoul(ram);
        return std::to_string(ram_val / 1024);
      }
    }
  }
  long ram_val = std::stoul(ram);
...
string LinuxParser::User(int pid)
{
  ...
  auto           uid     = static_cast<uid_t>(std::stoul(pid_uid));
  struct passwd* pwd     = getpwuid(uid);

  string name(pwd->pw_name), cleaned_name{};
  ...
  // For some reason my computer doesn't have my actual user in the /etc/passwd
  // file but htop found the name fine so I used the `getpwuid()` function
  // instead.
}
